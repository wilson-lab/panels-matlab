...
% Patsy = [repmat([zeros(4,8), 7*ones(4,8)], 1, 3), repmat([7*ones(4,8), zeros(4,8)], 1, 3)];
...
Pats(:,:,j,1) = Pats(:,:,j,1) - Patsy;
  ...
  for g = 1:pattern.y_num ;
    for i = 1:pattern.x_num 
        Pats(:,:,i,g) = ShiftMatrix(Pats(:,:,i,g), 32, 'r', 'y'); 
    end
  end
...
str = [directory_name '\Pattern_expansion_off_foeright_48_RC_telethon'];
save(str, 'pattern');
