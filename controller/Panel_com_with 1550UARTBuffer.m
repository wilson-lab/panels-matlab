...
switch lower(command)
    ...
    case 'flash_panel'
        ...
        %	set the trigger rate on the controller; send 0x06 and value
        %	0-255 (multiplied by 2 on the controller).
        send_serial( char([2 7 argument(1)]), 2);
    ...
    case 'eeprom_panel'
        ...
        %	set the trigger rate on the controller; send 0x06 and value
        %	0-255 (multiplied by 2 on the controller).
        send_serial( char([2 8 argument(1)]), 2);
        % three byte commands:
    ...
    case 'dump_frame'
        ...
        % argument 3 is the number of the panels
        % argument 4 is the gray scale level
        % argument 5 is the flag for the row compression
        % argument 6 to the end is the frame data
        % for example:
        % Panel_com('dump_frame', [384, 48, 1, 0, round(rand(1,8*48)*255)]);
       if (argument(1)>1536)||(~isnumeric(argument))
        ...
        send_serial(char([50, signed_16Bit_to_char(argument(1)), argument(2), argument(3), argument(4), argument(5:end)]));
